...
for i in range(com_num):                                #Iterate thru all commands
    ...
    if 'intersection_update' in com:                    #Check if the command is intersection_update
        st_set.intersection_update(op_set)              
    if 'update' in com:                                 #Check if the command is update
        st_set.update(op_set)
    if 'symmetric_difference_update' in com:            #Check if the command is symmetric_difference_update
        st_set.symmetric_difference_update(op_set)
    if 'difference_update' in com:                      #Check if the command is difference_update
